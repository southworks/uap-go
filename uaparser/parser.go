...
var jobsUA chan string = make(chan string)
var jobsOs chan string = make(chan string)
var jobsDevice chan string = make(chan string)
var resultsUA chan *UserAgent = make(chan *UserAgent)
var resultsOs chan *Os = make(chan *Os)
var resultsDevice chan *Device = make(chan *Device)
...
func (parser *Parser) workerUA() {
	for j := range jobsUA {
		resultsUA <- parser.ParseUserAgent(j)
	}
}

func (parser *Parser) workerOs() {
	for j := range jobsOs {
		resultsOs <- parser.ParseOs(j)
	}
}

func (parser *Parser) workerDevice() {
	for j := range jobsDevice {
		resultsDevice <- parser.ParseDevice(j)
	}
}
...
func (parser *Parser) Parse(line string) *Client {
	...
	cli.UserAgent = <-resultsUA
	cli.Os = <-resultsOs
	cli.Device = <-resultsDevice
